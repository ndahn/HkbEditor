...
def run(
    ...
    anims = {
        "Front": (front, ctx.new_clip(front)),
        "Back": (back, ctx.new_clip(back)),
        "Left": (left, ctx.new_clip(left)),
        "Right": (right, ctx.new_clip(right)),
    }

    # Blending transition effects
    dur0 = ctx.find("name=Duration0")
    dur7 = ctx.find("name=Duration7")
    ...
    for direction, (anim, clip) in anims.items():
        cmsg = ctx.new_cmsg(
            anim, 
            name=f"{name}_{direction}_anim_CMSG",
            generators=[clip],
            enableScript=False,
            offsetType=CmsgOffsetType.IDLE_CATEGORY,
        )
    ...
    stance_blend = ctx.new_blender_generator(
